...
function areYouAltAlting(thePlayer, card1, target_id)
	...
	if not target_id then --IF WITHDRAW
		if playerCharID ~= tonumber(card1[3]) then
			local antiAltAlt = mysql:query_fetch_assoc("SELECT `charactername` FROM `characters` LEFT JOIN `accounts` ON `characters`.`account`=`accounts`.`id` WHERE `accounts`.`id`='"..toSQL(playerAccID).."' AND `characters`.`id`='"..toSQL(card1[3]).."' LIMIT 1") or false
			...
			local cardOwnerSerial = mysql:query_fetch_assoc(" SELECT `mtaserial` FROM accounts a LEFT JOIN characters c ON a.id = c.account WHERE c.id='"..toSQL(card1[3]).."' LIMIT 1") or false
	...
	elseif target_id then -- TRANSFER
		local antiAltAlt = mysql:query_fetch_assoc("SELECT `charactername` FROM `characters` LEFT JOIN `accounts` ON `characters`.`account`=`accounts`.`id` WHERE `accounts`.`id`='"..toSQL(playerAccID).."' AND `characters`.`id`='"..toSQL(target_id).."' LIMIT 1") or false
		if antiAltAlt and antiAltAlt["charactername"] and string.len(antiAltAlt["charactername"])>0 then --DETECTED ALT-ALT BETWEEN CHARS IN SAME ACCOUNT
			return "between alts"
		end
		...
		local cardOwnerSerial = mysql:query_fetch_assoc(" SELECT `mtaserial` FROM accounts a LEFT JOIN characters c ON a.id = c.account WHERE c.id='"..toSQL(target_id).."' LIMIT 1") or false
		local playerSerial = getPlayerSerial(thePlayer)
		if cardOwnerSerial and playerSerial and cardOwnerSerial == playerSerial then -- DETECTED ALT-ALT OVER ACCOUNTS ON THE SAME MTA SERIAL
			return "between chars over the same mtaserial"
		end
