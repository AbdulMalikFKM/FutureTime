...
function showGeneralServiceGUI(atm)
	...
	local loan =  mysql:query_fetch_assoc("SELECT `states`, `finalvalue` FROM `bank_loans` WHERE `charid` ='"..thePlayerID.."'")
	if loan and loan.states and loan.finalvalue ~= nil then
	loanState = tostring(loan.states)
	loanValue = tonumber(loan.finalvalue)
	end
	--outputDebugString(money)
	--outputDebugString(exports.global:getMoney(client))
	triggerClientEvent(client, "showBankUI", getRootElement(), isInFaction, isFactionLeader, money, deposit, limit, withdraw, factionType, loanState, loanValue) -- last parameter is withdraw
end
...
function mySalary (thePlayer)
local factionID = factionID or getElementData(source, "faction")
local theTeam = exports.pool:getElement("team", factionID)
local factionWages = getElementData(theTeam, "wages") or 200
local playerFactionRank = getElementData(source, "factionrank")
Salary = factionWages[playerFactionRank]
triggerClientEvent(source, "mySalary", source, Salary)
end
addEvent("getMySalary", true)
addEventHandler("getMySalary", root, mySalary)


--------------sql for loans-----------------

function loan (Type, phoneNo, ratb, Age)
local CharID = getElementData(source, "account:character:id")
--local CharID = 12
local interestRate = "10%"
	if Type == 1 then
		mysql:query_free("INSERT INTO `bank_loans` (`charid`, `phone`, `salary`, `age`, `loanvalue`, `interestrate`, `interestvalue`, `finalvalue`, `states`, `time`) VALUES ('".. mysql:escape_string(CharID) .."', "..mysql:escape_string(phoneNo)..", "..mysql:escape_string(ratb)..", "..mysql:escape_string(Age)..", "..mysql:escape_string("50000")..", '"..mysql:escape_string(interestRate).."', '"..mysql:escape_string("5000").."', '"..mysql:escape_string("55000").."', '"..mysql:escape_string("Unpayed").."', NOW() - interval 1 hour)")
		setElementData(source, "loans.states", "Unpayed")
	--	setElementData(source, "loans.value", 55000)
	elseif Type == 2 then
		mysql:query_free("INSERT INTO `bank_loans` (`charid`, `phone`, `salary`, `age`, `loanvalue`, `interestrate`, `interestvalue`, `finalvalue`, `states`, `time`) VALUES ('".. mysql:escape_string(CharID) .."', "..mysql:escape_string(phoneNo)..", "..mysql:escape_string(ratb)..", "..mysql:escape_string(Age)..", "..mysql:escape_string("100000")..", '"..mysql:escape_string(interestRate).."', '"..mysql:escape_string("10000").."', '"..mysql:escape_string("110000").."', '"..mysql:escape_string("Unpayed").."', NOW() - interval 1 hour)")
		setElementData(source, "loans.states", "Unpayed")
	--	setElementData(source, "loans.value", 110000)
	elseif Type == 3 then
		mysql:query_free("INSERT INTO `bank_loans` (`charid`, `phone`, `salary`, `age`, `loanvalue`, `interestrate`, `interestvalue`, `finalvalue`, `states`, `time`) VALUES ('".. mysql:escape_string(CharID) .."', "..mysql:escape_string(phoneNo)..", "..mysql:escape_string(ratb)..", "..mysql:escape_string(Age)..", "..mysql:escape_string("500000")..", '"..mysql:escape_string(interestRate).."', '"..mysql:escape_string("50000").."', '"..mysql:escape_string("550000").."', '"..mysql:escape_string("Unpayed").."', NOW() - interval 1 hour)")
		setElementData(source, "loans.states", "Unpayed")
	--	setElementData(source, "loans.value", 550000)
	end
end
addEvent("giveMeloan", true)
addEventHandler("giveMeloan", root, loan)
--		mysql:query_free("INSERT INTO `bank_loans` (`charid`, `phone`, `salary`, `age`, `loanvalue`, `interestrate`, `interestvalue`, `finalvalue`, `states`, `time`) VALUES ('".. mysql:escape_string(CharID) .."', "..mysql:escape_string(phoneNo)..", "..mysql:escape_string(ratb)..", "..mysql:escape_string(Age)..", "..mysql:escape_string("50,000")..", '"..mysql:escape_string(interestRate).."', '"..mysql:escape_string("5,000").."', '"..mysql:escape_string("55,000").."', '"..mysql:escape_string("Unpayed").."', NOW() - interval 1 hour)")


function checkLoans ()
			local result = mysql:query( "SELECT * FROM `bank_loans` ORDER BY id ASC" )
		if result then
			local loans = { }
			...
			while true do
				...
				table.insert( loans, { row.id, row.charid, row.phone, row.salary, row.age, row.loanvalue, row.interestrate, row.interestvalue, row.finalvalue, row.states, row.time} )
				
			end
			
			mysql:free_result(result)	
			triggerClientEvent( source, "ShowLoansGUI", source, loans)		
			else
			outputChatBox("Error", source, 255, 0, 0)
		end
end
addEvent("ShowAllLoans", true)
addEventHandler( "ShowAllLoans", getRootElement(), checkLoans )