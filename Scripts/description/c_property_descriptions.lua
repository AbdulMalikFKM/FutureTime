		...
		if isElement(theProperty) then
			...
			if getDistanceBetweenPoints3D(cx,cy,cz,x,y,z) <= viewDistance then --Within radius viewDistance
				...
				if px and isLineOfSightClear(cx, cy, cz, x, y, z, true, false, false, true, true, false, false) then	
					...
					toBeAdded = pname
					toBeShowed = "#C8C8C8" ..toBeShowed..pname.."\n"
					...
					if powner < 1 and pfaction < 1 and ptype ~= 2 then -- IF FOR SALE
						if ptype == 3 then -- RENTABLE
							toBeAdded = "#C8C8C8For lease: #537374$"..exports.global:formatMoney(pcost).."\n"
							toBeShowed = toBeShowed..toBeAdded
						if can >= pcost then
						toBeShowed = toBeShowed.."#C8C8C8You Can Buy : #00FF00"..yes.."\n" end
						if can < pcost then
						toBeShowed = toBeShowed.."#C8C8C8You Can Buy : #FF0000"..no.."\n" end
							local len = dxGetTextWidth(toBeAdded)
							if len > fontWidth then
								fontWidth = len
							end
							...
							toBeAdded = "#C8C8C8For sale: #537374$"..exports.global:formatMoney(pcost).."\n"
							toBeShowed = toBeShowed..toBeAdded
						if can >= pcost then
						toBeShowed = toBeShowed.."#C8C8C8You Can Buy : #00FF00"..yes.."\n" end
						if can < pcost then
						...
						if exports.integration:isPlayerTrialAdmin(localPlayer) and (getElementData(localPlayer, "duty_admin") == 1) then
							...
							toBeAdded = "Owner: #537374"..(ownerName or "#537374Loading..").."\n"
							toBeShowed = toBeShowed..toBeAdded
							local len = dxGetTextWidth(toBeAdded)
							if len > fontWidth then
								fontWidth = len
				    ...
				    if exports['interior-system']:canPlayerSeeInteriorID(theProperty) then
						toBeAdded = "#BB0001(( ID: #537374"..getElementData(theProperty,"dbid").." ))\n"
						toBeShowed = toBeShowed..toBeAdded
						local len = dxGetTextWidth(toBeAdded)
						if len > fontWidth then
							fontWidth = len
					...
					if getElementData(localPlayer, "bgPro") ~= "0" then
						roundedRectangle(px-marg, py-marg, fontWidth+(marg*2), fontHeight+(marg*2), tocolor(0, 0, 0, 100))
					end
					...
					dxDrawText(toBeShowed, px, py, px + fontWidth, (py + fontHeight), textColor, 1, fontElement, "center", "center", false, false, false, true, false)
				end
			end
		end
	end
