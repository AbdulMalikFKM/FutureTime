		...
		if (menuVisible==0) then
			...
			if (factionID~=-1) then
				...
				if query then
					...
					local mQuery = mysql:query("SELECT 'statue' FROM `factions` WHERE `id` = '"..mysql:escape_string(factionID).."'")
					while true do
					row = mysql:fetch_assoc(mQuery)
						if not row then break end
						statue = row.statue
...
--Edited By Dharma --\ This Thing to make sure the rank and wage update bc they change from Editbox to Gridlist.
function callbackUpdateRanks(ranks, wages, sranks, swages, fRanks, fWages)
	...
	if not hasPlayerAccessOverFaction(client, factionID) then
		...
		if (wages) then
				wages = math.min(2500, math.max(0, tonumber(wages) or 0))
				
		mysql:query_free("UPDATE factions SET wage_"..swages.."='" .. wages .. "' WHERE id='" .. factionID .. "'")
		end
		
	mysql:query_free("UPDATE factions SET rank_"..sranks.."='" .. ranks .. "' WHERE id='" .. factionID .. "'")
	
	outputChatBox("Faction information updated successfully.", source, 0, 255, 0)

	showFactionMenu(source)
			local result = mysql:query("SELECT * FROM `factions` WHERE id ='" ..factionID.."'")
	if not result then return end
...
function callbackUpdateMOTD(motd)
	...
	if (factionID~=-1) then
		if mysql:query_free("UPDATE factions SET motd='" .. tostring(mysql:escape_string(motd)) .. "' WHERE id='" .. factionID .. "'") then
			outputChatBox("You changed your faction's radio frequncey to '" .. motd .. "'", client, 0, 255, 0)
			exports.anticheat:changeProtectedElementDataEx(theTeam, "motd", motd, false)
		else
...
function ShowApp(faction)
	local information = { }
	local mQuery = mysql:query("SELECT * FROM `jobapp` WHERE `jobname` ='" ..mysql:escape_string(faction).."'")
		while true do
			local row = mysql:fetch_assoc(mQuery)
				if not row then break end
				table.insert( information, {row.id, row.name, row.phone, row.age, row.address, row.email, row.description, row.rp, row.time} )				
						triggerClientEvent( source, "JobApp:ShowAppAdminstrator", source, information)
			--	end
						end
...
addEvent("JobApp:ShowApp", true)
addEventHandler( "JobApp:ShowApp", getRootElement(), ShowApp )

function deleteApp(appID)
	mysql:query_free("DELETE FROM jobapp WHERE id="..appID)
end
addEvent("JobApp:DeleteApp", true)
addEventHandler("JobApp:DeleteApp", resourceRoot, deleteApp)

function updateStatue (statue, faction) 
	mysql:query_free("UPDATE `factions` SET `statue` = " ..statue .. " WHERE `name` = '"..mysql:escape_string(faction).."'")			
end
addEvent("JobApp:StatueAdminstrator", true)
addEventHandler( "JobApp:StatueAdminstrator", getRootElement(), updateStatue )