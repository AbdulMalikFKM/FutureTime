...
function getInteractions(element)
	...
	table.insert(interactions, {"Close", "icons/cross_x.png",
		function ()
			Interaction.Close()
		end
	})
		...
		if exports.sarp_groups:isPlayerHavePermission(localPlayer, "cuff") then
			if getElementData(element, "player.Cuffed") then
				if getElementData(element, "player.Grabbed") then
					table.insert(interactions, {"Uncuff", "icons/uncuff.png",
						function (player, target)
							triggerServerEvent("grabPlayerOff", localPlayer, target)
						end
					})
				else
					table.insert(interactions, {"Cuff", "icons/cuff.png",
						function (player, target)
							triggerServerEvent("grabPlayer", localPlayer, target)
						end
					})

					table.insert(interactions, {"Remove Clamp", "icons/uncuff.png",
						function (player, target)
		...
		table.insert(interactions, {"Csomagtartó", "icons/trunk.png",
			function (player, target)
				if getPedOccupiedVehicle(localPlayer) then
					exports.notification:addNotification("You cannot look into the boot while sitting in a vehicle!", "error")
				else
					if getVehicleType(element) == "Automobile" then
						if not exports.sarp_inventory:bootCheck(element) then
							exports.notification:addNotification("You can only look into the luggage compartment while standing at the trunk of the vehicle!", "error")
							return
						end
					end
				...
				table.insert(interactions, {"Kerékbilincs leszedése", "icons/wheelclamp.png",
					function (player, target)
						...
						if getDistanceBetweenPoints3D(cX, cY, cZ, getElementPosition(localPlayer)) <= 1.5 then
							...
							exports.notification:addNotification("You are too far from the left front wheel", "error")
						end
					end
				})
			else
	...
	elseif type == "object" then
		if getElementData(element, "isFactoryObject") then
			local tempActions = exports['job-system']:getCurrentInteractionList(model)
		...
		elseif getElementData(element, "isFuelPump") then
			...
			tempActions = nil
		elseif getElementData(element, "isContainerObject") then
			local tempActions = exports['job-system']:getCurrentInteractionListsweeper(model)
			...
			tempActions = nil
		elseif model == exports['job-system']:getElevatorID() then
