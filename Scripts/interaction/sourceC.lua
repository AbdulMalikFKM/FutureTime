...
local UI = exports.global
local responsiveMultipler = UI:getResponsiveMultiplier()

function loadFonts()
    fonts = {
        Roboto11 = "default-bold",
        Roboto14 = "default-bold",
        Roboto16 = "default-bold",
        Roboto18 = "default-bold",
        RobotoL = "default-bold",
        RobotoL14 = "default-bold",
        RobotoL16 = "default-bold",
        RobotoL18 = "default-bold",
        RobotoL24 = "default-bold",
        RobotoLI16 = "default-bold",
        RobotoLI24 = "default-bold",
        RobotoB18 = "default-bold",
    }
end
...
Interaction.Panel = function(interactions)
    if interactionActive then
        ...
        if getDistanceBetweenPoints3D(px, py, pz, ex, ey, ez) > getElementRadius(selectedElement) * 4 then
            Interaction.Close()
            return
        end
        ...
        if getElementType(selectedElement) == "vehicle" then
            local textW = dxGetTextWidth(exports.global:getVehicleName(selectedElement), 1, fonts.Roboto14)
       ...
       -- dxDrawRectangle(panelX, panelY, panelW, panelH + (rowH * adjustH), tocolor(31, 31, 31, 240))
        ...
        if getElementType(selectedElement) == "vehicle" then
           -- dxDrawText(exports.global:getVehicleName(selectedElement), panelX, panelY + respc(10), panelW + panelX, panelH + panelY, tocolor(255, 255, 255, 255), 1, fonts.Roboto14, "center", "top")
        ...
        if getElementType(selectedElement) == "vehicle" then
           -- dxDrawText("(Vehicle)", panelX, panelY + respc(10) + dxGetFontHeight(1, fonts.Roboto14), panelW + panelX, panelH + panelY, tocolor(255, 255, 255, 255), 1, fonts.Roboto11, "center", "top")
        elseif getElementType(selectedElement) == "player" then
           -- dxDrawText("(Player)", panelX, panelY + respc(10) + dxGetFontHeight(1, fonts.Roboto14), panelW + panelX, panelH + panelY, tocolor(255, 255, 255, 255), 1, fonts.Roboto11, "center", "top")
        elseif getElementType(selectedElement) == "ped" then
        ...
        elseif getElementType(selectedElement) == "object" then
           -- dxDrawText("(Object)", panelX, panelY + respc(10) + dxGetFontHeight(1, fonts.Roboto14), panelW + panelX, panelH + panelY, tocolor(255, 255, 255, 255), 1, fonts.Roboto11, "center", "top")
        end
        --if 
...
addEventHandler("onClientClick", root, function(button, state, absoluteX, absoluteY, worldX, worldY, worldZ, clickedElement)
    if button == "right" and state == "down" then
        if not interactionActive then
            ...
            if isElement(clickedElement) then
                if clickedElement ~= localPlayer then
                    if getElementData(clickedElement, "isInteractable") or getElementType(clickedElement) == "vehicle" or getElementType(clickedElement) == "player" then
                        if exports.global:inDistance3D(clickedElement, localPlayer, getElementRadius(clickedElement) * 4) then
            ...
            exports.notification:addNotification("You are already open a task press [M] if there any problem", "error")
        end
    elseif button == "left" and state == "down" then
        if interactionActive then
            for i = 1, #elementInteractions do
                if activeButton == "interaction:" .. i then
                    playSound("icons/click.ogg")
