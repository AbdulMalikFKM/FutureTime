...
local jessie = createPed( 141, 1529.4638671875, -1834.9326171875, 13.546875 )
setPedRotation( jessie, 32 )
...
function showEmploymentWindow()
	...
	local row = guiGridListAddRow(jobList)
	guiGridListSetItemText(jobList, row, column, "Store Keeper", false, false)

	local row = guiGridListAddRow(jobList)
	guiGridListSetItemText(jobList, row, column, "Window Cleaner", false, false)
	...
	if (button=="left") then
		...
		elseif (job>0) then
			...
			elseif (jobtext=="Locksmith") then
				displayLocksmithJob()
				job = 6
			elseif (jobtext=="Store Keeper") then
				displaySKJob()
				job = 8
			elseif (jobtext=="Window Cleaner") then
				displayWCJob()
				job = 9
			end
