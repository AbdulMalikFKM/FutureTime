...
function displaySKJob()
	exports.hud:sendBottomNotification(localPlayer, "Store Keeper", "Approach The Yellow Blip There's Man Will Explain You.")
end

local function loadFonts()
	local fonts = {

		ocr10bold = dxCreateFont("storekeeper/files/ocr.ttf", 10),
		ocr25bold = dxCreateFont("storekeeper/files/ocr.ttf", 25),
		RobotoLabel = dxCreateFont("storekeeper/files/Roboto-Regular.ttf", 14),
		RobotoBold = dxCreateFont("storekeeper/files/Roboto-Bold.ttf", 24),
		handFont = dxCreateFont("storekeeper/files/hand.otf", 24),
		barCode = dxCreateFont("storekeeper/files/VT323.ttf", 10),
		CourierNew = dxCreateFont("storekeeper/files/CourierNew.ttf", 18),
		RobotoBoldR = dxCreateFont("storekeeper/files/Roboto-Bold.ttf", 24),
		lunabar = dxCreateFont("storekeeper/files/lunabar.ttf", 26),
	}
...
local sign1 = dxCreateTexture("storekeeper/files/sign1.png")
local sign2 = dxCreateTexture("storekeeper/files/sign2.png")
local sign3 = dxCreateTexture("storekeeper/files/sign3.png")
local stripe = dxCreateTexture("storekeeper/files/stripe.png")

local selection = dxCreateTexture("storekeeper/files/selection.png")
...
function applyTextureToElement(element, texture, applyTo)
	if isElement(element) and isElement(texture) then
		...
		texturedElements[element][1] = dxCreateShader("storekeeper/files/texturechanger.fx")
...
function startTheJob()
	...
	for i = 1, jobData.shelfPlacedItems do
		...
		setElementData(boxObject, "object.name", "Package")
	...
	setElementData(mainCarrierObject, "object.name", "Carrier")
	setElementData(mainCarrierObject, "isFactoryObject", true)

	currentInteractions.carrier = {}
	currentInteractions.crate = {}

	table.insert(currentInteractions.carrier, {"Pick Up Carrier", "storekeeper/files/pickUpCarrier.png", "onClientPlayerPickUpCarrier"})

	table.insert(currentInteractions.crate, {"Pick Up Crate", "storekeeper/files/moveCrate.png", "onClientPlayerPickUpCrate"})
end
...
function endJob(finish)
	...
	if finish then
		local payment = crateCount

		exports.notification:addNotification("You have finished the job successfully! payment: " .. payment .. " $", "success")
		--outputChatBox(exports.sarp_core:getServerTag("info") .. "Elvégezted a munkád, a fizetésed: #acd373" .. payment .. " $", 50, 179, 239, true)
		outputChatBox("You did your job, your salary #acd373" .. payment .. " $", 50, 179, 239, true)
...
addEventHandler("onClientClick", getRootElement(),
	function (button, state, absoluteX, absoluteY, worldX, worldY, worldZ, clickedWorld)
		if state == "up" and clickedWorld == jobPed and getElementData(localPlayer, "job") == JOB_ID then
			...
			if getDistanceBetweenPoints3D(playerPosX, playerPosY, playerPosZ, pedPosX, pedPosY, pedPosZ) <= 5 then
				if jobStarted then
					exports.notification:addNotification("You're already working!", "error")
				else
					startTheJob()

					outputChatBox("You started work. You will find the hand pallet truck at the entrance to the warehouse.", 255, 150, 0, true)
					outputChatBox("Currently #32b3ef" .. jobData.shelfPlacedItems .. " Crate#ffffff you need to ship the package.", 255, 150, 0, true)

					exports.notification:addNotification("You started work.", "info")
				end
			end
		end
	end
...
addEventHandler("onClientPlayerPickUpCarrier", getRootElement(),
	function (sourceElement)
		...
		toggleControl({"jump", "aim_weapon", "fire", "enter_exit", "crouch", "sprint"}, false)
	end
)
...
addEventHandler("onClientPlayerPickUpCrate", getRootElement(),
	function (sourceElement)
		if not getElementData(localPlayer, "carrierActive") then
			exports.notification:addNotification("Pick up the pallet truck from the warehouse entrance first!", "error")
		else
			local cratesOnCarrier = getElementData(localPlayer, "carrierPlacedObjects") or {}

			if #cratesOnCarrier + 1 > 4 then
				exports.notification:addNotification("No more crates on the jack!", "error")
			else
...
addEventHandler("onClientPlayerPickUpCarrierAfterDropped", getRootElement(),
	function (sourceElement)
		if jobData.carryingCrate then
			exports.notification:addNotification("First, pick the box in your hand!", "error")
		else
...
addEventHandler("onClientPlayerPickUpCrateFromCarrier", getRootElement(),
	function (sourceElement)
		if getElementData(localPlayer, "carryingCrate") then
			exports.notification:addNotification("You already have a crate in your hand!", "error")
		else
			setElementData(sourceElement, "isInteractable", false)
...
addEventHandler("onClientElementDataChange", getRootElement(),
	function (dataName, oldValue)
		...
		elseif dataName == "donePackages" then
			updateQuantity()
		elseif dataName == "job" and source == localPlayer then
			endJob()
		end
	end
)
	...
	if getElementData(player, "carrierActive") then
		...
		if getElementData(player, "carrierPlacedOnFactory") and syncableCarriers[player] then
			...
			if player == localPlayer then
				...
				setElementData(syncableCarriers[player], "object.name", "Pick Up Carrier")
				setElementData(syncableCarriers[player], "isFactoryObject", true)

				currentInteractions.carrier = {}
				table.insert(currentInteractions.carrier, {"Pick Up", "storekeeper/files/pickUpCarrier.png", "onClientPlayerPickUpCarrierAfterDropped"})
			end
			...
			if getElementData(player, "carryingCrate") and not syncableCarryingCrates[player] then
				...
				setObjectScale(crateObject, 0.9)
				 ...
				 exports.bone_attach:attachElementToBone(crateObject, player, 12, 0.25, 0, 0.15, -100, 0, -20)
			...
			for i = 1, #carrierPlacedObjects do
				...
				if player == localPlayer then
					...
					if enableCrateCollisions then
						currentInteractions.carrier = {}
						table.insert(currentInteractions.carrier, {"Pick Up", "storekeeper/files/pickUpCarrier.png", "onClientPlayerPickUpCarrierAfterDropped"})
						table.insert(currentInteractions.carrier, {"Add Box", "storekeeper/files/pickUpCrate.png", "onClientPlayerPickUpCrateFromCarrier"})
					end
				end
			end
		end
...
function PickUpCarrier(button, state, absX, absY, wx, wy, wz, element)
			if state ~= "down" or button ~= "right" then
			...
			local row = {}
			 ...
			 if getElementModel(element) == carrierModel then
			 rcMenu = exports['rightclick']:create("                 Carrier(Object)                 ")
			 row.use = exports['rightclick']:addRow("Pick up Carrier")
			 DGS:dgsSetProperty(row.use,"iconImage",{":job-system/storekeeper/files/pickUpCarrier.png",":job-system/storekeeper/files/pickUpCarrier.png",":job-system/storekeeper/files/pickUpCarrier.png"})
			 DGS:dgsSetProperty(row.use,"iconDirection","left")
			 row.box = exports['rightclick']:addRow("Pick up Box")
			 DGS:dgsSetProperty(row.box,"iconImage",{":job-system/storekeeper/files/moveCrate.png",":job-system/storekeeper/files/moveCrate.png",":job-system/storekeeper/files/moveCrate.png"})
			 DGS:dgsSetProperty(row.box,"iconDirection","left")
			 addEventHandler("onDgsMouseClickUp", row.use, function()
				triggerEvent("onClientPlayerPickUpCarrierAfterDropped", localPlayer, element)	
			 end, false)
			addEventHandler("onDgsMouseClickUp", row.box, function()
			triggerEvent("onClientPlayerPickUpCrateFromCarrier", localPlayer, element)	
			 end, false)
			 row.close = exports['rightclick']:addRow("Close")
			 DGS:dgsSetProperty(row.close,"iconImage",{":interaction/icons/cross_x.png",":interaction/icons/cross_x.png",":interaction/icons/cross_x.png"})
			 DGS:dgsSetProperty(row.close,"iconDirection","left")
			 end
			 end
	
end
addEventHandler("onClientClick", getRootElement(), PickUpCarrier)
...
function PickUpCarrier(button, state, absX, absY, wx, wy, wz, element)
			...
			local row = {}
              if element and getElementType(element)=="object" then
			 if getElementModel(element) == crateModel then
			 rcMenu = exports['rightclick']:create("                 Crate(Object)                 ")
			 row.use = exports['rightclick']:addRow("Pick up")
			 DGS:dgsSetProperty(row.use,"iconImage",{":job-system/storekeeper/files/pickUpCrate.png",":job-system/storekeeper/files/pickUpCrate.png",":job-system/storekeeper/files/pickUpCrate.png"})
			 DGS:dgsSetProperty(row.use,"iconDirection","left")
			 addEventHandler("onDgsMouseClickUp", row.use, function()
				triggerEvent("onClientPlayerPickUpCrate", localPlayer, element)	
			 end, false)
			 row.close = exports['rightclick']:addRow("Close")
			 DGS:dgsSetProperty(row.close,"iconImage",{":interaction/icons/cross_x.png",":interaction/icons/cross_x.png",":interaction/icons/cross_x.png"})
			 DGS:dgsSetProperty(row.close,"iconDirection","left")
...
function renderInsideFactory()
	...
	if jobStarted and isElementWithinColShape(localPlayer, labelingColshape) then
		...
		dxDrawImage(x, y, sx, sy, "storekeeper/files/notes.png")
		...
		if jobData.carryingCrate then
			if jobData.carryingCrate.canPlace then
				dxDrawText("To put the object press [E] button.", 1, screenY - respc(256) + 1, screenX + 1, screenY - respc(224) + 1, tocolor(0, 0, 0), 1, RobotoLabel, "center", "center")
				dxDrawText("To put the object press [E] button.", -1, screenY - respc(256) - 1, screenX - 1, screenY - respc(224) - 1, tocolor(0, 0, 0), 1, RobotoLabel, "center", "center")
				dxDrawText("To put the object press [E] button.", -1, screenY - respc(256) + 1, screenX - 1, screenY - respc(224) + 1, tocolor(0, 0, 0), 1, RobotoLabel, "center", "center")
				dxDrawText("To put the object press [E] button.", 1, screenY - respc(256) - 1, screenX + 1, screenY - respc(224) - 1, tocolor(0, 0, 0), 1, RobotoLabel, "center", "center")
				dxDrawText("To put the object #32b3ef[E]#ffffff button.", 0, screenY - respc(256), screenX, screenY - respc(224), tocolor(255, 255, 255), 1, RobotoLabel, "center", "center", false, false, false, true)
			elseif not showSticker then
				dxDrawText("To put the crate press [E] button.", 1, screenY - respc(256) + 1, screenX + 1, screenY - respc(224) + 1, tocolor(0, 0, 0), 1, RobotoLabel, "center", "center")
				dxDrawText("To put the crate press [E] button.", -1, screenY - respc(256) - 1, screenX - 1, screenY - respc(224) - 1, tocolor(0, 0, 0), 1, RobotoLabel, "center", "center")
				dxDrawText("To put the crate press [E] button.", -1, screenY - respc(256) + 1, screenX - 1, screenY - respc(224) + 1, tocolor(0, 0, 0), 1, RobotoLabel, "center", "center")
				dxDrawText("To put the crate press [E] button.", 1, screenY - respc(256) - 1, screenX + 1, screenY - respc(224) - 1, tocolor(0, 0, 0), 1, RobotoLabel, "center", "center")
				dxDrawText("To put the crate press #32b3ef[E]#ffffff button.", 0, screenY - respc(256), screenX, screenY - respc(224), tocolor(255, 255, 255), 1, RobotoLabel, "center", "center", false, false, false, true)
			else
				sx = respc(822)
				sy = respc(417)
				x = (screenX - sx) / 2
				...
				dxDrawImage(x, y, sx, sy, "storekeeper/files/sticker.png")
				...
				dxDrawText("\"HOME\" STORAGE BOX | 1 PIECE", x + respc(41), y + respc(171), x + respc(562), y + respc(210), tocolor(0, 0, 0), 0.75, CourierNew, "center", "center")
				...
				if signStart then
					...
					if progress > 1.25 then
						...
						exports.notification:addNotification("Move the crate to the place marked in yellow.", "error")
					end
				end
			end
		elseif syncableCarriers[localPlayer] and isElementAttached(syncableCarriers[localPlayer]) then
			...
			dxDrawText("To put the object press [E] button.", -1, screenY - respc(256) + 1, screenX - 1, screenY - respc(224) + 1, tocolor(0, 0, 0), 1, RobotoLabel, "center", "center")
			dxDrawText("To put the object press [E] button.", 1, screenY - respc(256) - 1, screenX + 1, screenY - respc(224) - 1, tocolor(0, 0, 0), 1, RobotoLabel, "center", "center")
			dxDrawText("To put the object press #32b3ef[E]#ffffff button.", 0, screenY - respc(256), screenX, screenY - respc(224), tocolor(255, 255, 255), 1, RobotoLabel, "center", "center", false, false, false, true)
		end
	end
end
		...
		if jobStarted and not showSticker then
			if isElementWithinColShape(localPlayer, labelingColshape) then
				if syncableCarriers[localPlayer] and isElementAttached(syncableCarriers[localPlayer]) and canPlaceObject(syncableCarriers[localPlayer]) then
					...
					toggleControl({"jump", "aim_weapon", "fire", "enter_exit", "crouch", "sprint"}, true)
				elseif jobData.carryingCrate then
					if jobData.carryingCrate.canPlace then
						if isElementWithinColShape(localPlayer, crateFinalPosition[7]) then
							...
							exports.notification:addNotification("You can't put it down here, go to the place marked in yellow!", "error")
						end
					else
						...
						if not workbench then
							exports.notification:addNotification("You can't put it here go to a table!", "error")
						else
							local playerX, playerY, playerZ = getElementPosition(localPlayer)
							...
							if hit and hitElement and getElementModel(hitElement) == 941 then
								...
								exports.notification:addNotification("Face the table or stand closer!", "error")
							end
						end
					end
				end
			...
			if fakeInputs["customerName"] == jobData.carryingCrate.customerName then
				if fakeInputs["deliveryAddress"] == jobData.carryingCrate.address then
					if tonumber(fakeInputs["zipCode"]) == jobData.carryingCrate.zipCode then
						...
						playSound("storekeeper/files/sign.mp3")
					else
						exports.notification:addNotification("The zip code is incorrect!", "error")
					end
				else
					exports.notification:addNotification("The customer's address is incorrect!", "error")
				end
			else
				exports.notification:addNotification("The customer name is incorrect!", "error")
			end
		...
		if isElement(tempRT) then
			...
			for x = 0, position[4] * 2 do
				for y = 0, position[5] * 2 do
					dxDrawImage(x * 24, y * 24, 24, 24, "storekeeper/files/stripe.png")
				end
			end
	...
	if isElement(objectElement) then
		...
		for i = 0, 1 do
			...
			if not isLineOfSightClear(objectX, objectY, objectZ, offX, offY, offZ, true, true, false, true, true, false, false, localPlayer) then
				exports.notification:addNotification("You can't put it down here because the object collides with something!", "error")
				return
			end
