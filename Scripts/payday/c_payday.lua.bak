function cPayDay(faction, pay, profit, interest, donatormoney, tax, incomeTax, vtax, ptax, rent, grossincome, Perc, states, value)
...
--	for i, v in ipairs (loans) do
		--if getElementData(localPlayer, "loans.states") == false then
		--if states == "Unpayed" then
		if (value > 0) then
			--if (loans[i][2] == 55000) then
				cal = grossincome * 0.10
				--outputChatBox(,255, 194, 14, true)
				table.insert(info, {"  Bank Loans: $" .. value - cal})
			--end
		--end
	end
...
--	elseif (states == "Unpayed") then
--		table.insert(info, {"  Gross Income: $" .. grossincome - (grossincome * 0.10)})
	--	table.insert(info, {"  Remark(s): Transferred to your bank account."})
	else
		--outputChatBox(, 255, 194, 14, true)
		--outputChatBox(, 255, 194, 14)
