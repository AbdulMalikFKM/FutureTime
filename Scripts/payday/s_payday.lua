...
function payWage(player, pay, faction, tax)
	...
	local thePlayerID = getElementData(player, "account:character:id")
	...
	local mQuery =  mysql:query_fetch_assoc("SELECT `states`, `finalvalue` FROM `bank_loans` WHERE `charid` ='"..thePlayerID.."'")
		loanState = tostring(mQuery.states)
		loanValue = tonumber(mQuery.finalvalue)
	...
	local loanfinalValue = loanValue - (grossincome * 0.10)
	triggerClientEvent(player, "cPayDay", player, faction, noWage and -1 or pay, profit, interest, donatormoney, tax, incomeTax, vtax, ptax, rent, grossincome, cP, loanState, loanValue)
	
	mysql:query_free("UPDATE bank_loans SET finalvalue='" .. tostring(mysql:escape_string(loanfinalValue)) .. "' WHERE charid='" .. thePlayerID .. "'")
	
	if loanfinalValue <= 0 or loanValue <= 0 then
		mysql:query_free("UPDATE bank_loans SET finalvalue='" .. tostring(mysql:escape_string("0")) .. "', states='" ..tostring(mysql:escape_string("Payed")) .. "'  WHERE charid='" .. thePlayerID .. "'")
	else
		mysql:query_free("UPDATE bank_loans SET finalvalue='" .. tostring(mysql:escape_string(loanfinalValue)) .. "' WHERE charid='" .. thePlayerID .. "'")
		setElementData(player, "bankmoney", math.floor(tonumber(bankmoney) - (grossincome * 0.10)))
	end
	--2001, 2134, 2192, 2238, 2277 = 10842
	--200.1, 213.4, 219.2, 223.8, 227.7 = 1084.2

	--2000, 2127, 2182, 2226, 2262 = 10797
	--200, 212.7, 218.2, 222.6, 226.2 = 1079.7
	--9942
